...
def most_popular_articles(dbq):
    q = (
        ...
        "(select log.path, count(*) as num from log where log.path like '/article/%' "
...
def most_popular_authors(dbq):
    q = (
       ...
       "where log.path like '/article/%' and log.method = 'GET' group by log.path) "
       "as reqs, articles where reqs.path like concat('/article/', articles.slug) "
...
def http_error_days(dbq):
    q = (
        "select total_reqs.date, cast(bad_reqs.total_bad as decimal)/cast(total_reqs.total as decimal) "
